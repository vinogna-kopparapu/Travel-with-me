...
public class BookTransportation extends JFrame implements ActionListener {
    ...
    BookTransportation(String username) {
        ...
        try {
            Conns c = new Conns();
            ResultSet rs = c.stmt.executeQuery("Select * from transport");
            while (rs.next()) {
        ...
        JLabel l5 = new JLabel("Number of Hours");
        ...
        JLabel l7 = new JLabel("Self-Drive");
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == b1) {
            try {
                Conns c = new Conns();
                ResultSet rs = c.stmt.executeQuery("select * from transport where name = '" + c1.getSelectedItem() + "'");
                while (rs.next()) {
                    int cost = Integer.parseInt(rs.getString("costperperson"));
                    int driver = Integer.parseInt(rs.getString("driverincluded"));
                    int ac = Integer.parseInt(rs.getString("accar"));

                    int person = Integer.parseInt(t1.getText());
                    int hours = Integer.parseInt(t2.getText());

                    String acprice = c2.getSelectedItem();

                    String driverprice = c3.getSelectedItem();

                    if (person * hours > 0) {
                        int total = 0;
                        total += acprice.equals("YES") ? ac : 0;

                        total += driverprice.equals("YES") ? driver : 0;

                        total += cost;

                        total += total * person * hours;
            ...
            try {
                Conns c = new Conns();
                c.stmt.executeUpdate("insert into booktransport values('"+l21.getText()+"','"+c1.getSelectedItem()+"','"+t1.getText()+"','"+t2.getText()+"','"+c2.getSelectedItem()+"','"+c3.getSelectedItem()+"','"+l22.getText()+"','"+l23.getText()+"','"+l24.getText()+"','"+l25.getText()+"')");

                JOptionPane.showMessageDialog(null, "Treansport booked Successfully");
